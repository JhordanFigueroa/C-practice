...
static void change_cd(char *filename, int cdnum) {
	...
	if (f == 0) {
		...
		cdptr->trackcount = tempcd.trackcount;              //-> assignment operator - for the purpose of assessing the fields of a struct - its a way to point to it  -> used to reference fields of a pointer to a filed in that struct 
...
static int load_cdcollection(char *filename) {
	...
	if (f == 0) {
        ...
        //realloc - allows you to change the amount of memory needed for all the data 
